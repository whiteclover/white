...
def page_edit(page_id):
    ...
    page = page_service.update_page(parent, name, title, slug, content, status, redirect_, show_in_menu, page_id)
...
def page_add():
    ...
    validator = Validator()
    validator.add('duplicate', lambda key: page_service.is_exist_slug(key) == False)
    (validator
        .check(title, 'min', text('page.title_missing'), 3)
    ...
    page = page_service.add_page(parent, name, title, slug, content, status, redirect_, show_in_menu)
    extend_service.prcoess_field(page, 'page')
